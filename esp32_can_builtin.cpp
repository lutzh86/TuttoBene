...
bool ESP32CAN::processFrame(CAN_frame_t &frame)
{
    ...
    for (int i = 0; i < 8; i++) msg.data.bytes[i] = frame.data.u8[i];
...
bool ESP32CAN::sendFrame(CAN_FRAME& txFrame)
{
    ...
    for (int i = 0; i < 8; i++) __TX_frame.data.u8[i] = txFrame.data.bytes[i];
...
uint32_t ESP32CAN::get_rx_buff(CAN_FRAME &msg)
{
    ...
    if(xQueueReceive(CAN_cfg.rx_queue,&__RX_frame, 0)==pdTRUE)
    {
        ...
        for (int i = 0; i < 8; i++) msg.data.bytes[i] = __RX_frame.data.u8[i];
        return true;
    }
    return false;
}
